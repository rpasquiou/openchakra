...
const USER_ACTIONS={
  [FEURST_ADMIN]: lodash.flattenDeep([
    ...
    [VIEW, CREATE, IMPORT].map(action => createUserAction(SHIPRATE, action, {visibility: ALL})),
  ...
  [CUSTOMER_ADMIN]: lodash.flattenDeep([
    ...
    [CREATE, VIEW, UPDATE, VALIDATE, DELETE, CONVERT].map(action => createUserAction(ORDER, action, {visibility: COMPANY})),
  ]),
...
// Hours delay after removing non submitted quotations
const QUOTATION_LIFETIME_HOURS=3*24
...
module.exports={
  ...
  SHIPRATE, MAX_WEIGHT, PRODUCT, STANDARD_SHIPPING, EXPRESS_SHIPPING, GROUP_SHIPPING, BASEPATH_EDI, ALL, FEURST_PHONE_NUMBER, FEURST_EMAIL, FEURST_IMG_PATH, FEURST_ICON_PATH,
  COMPANY, RELATED,
  CREATED, VALID, PARTIALLY_HANDLED, HANDLED, SHIPPING_MODES,
  COMPLETE, LINK, UPDATE_CGV, FEURST_SALES, HANDLE, PRICELIST,
  ...
  ORDER_ALERT_DELAY, EXPORT, IMPORT,
  QUOTATION_LIFETIME_HOURS,
}
