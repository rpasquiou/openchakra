...
const postPutData = async ({model, id, user, attribute, value}) => {
  ...
  if (model == `group`) {
    if (attribute == 'users') {
      await Group.updateOne({_id:id}, {$pullAll: {pending_users: value}})
    }
  }
