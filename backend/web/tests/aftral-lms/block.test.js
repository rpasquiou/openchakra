...
describe('User', () => {
  ...
  it.only('must return resource session', async()=> {
    ...
    expect(resource.session.length).toEqual(1)
    expect(sequence.session.length).toEqual(1)
    expect(modulee.session.length).toEqual(1)
    expect(program.session.length).toEqual(1)
    expect(session.session.length).toEqual(0)
  })
})